...
import { CATEGORY_MAPPING, CATEGORY_STYLES, useSearchStore } from '@/lib/store/search';
...
const Search = () => {
  ...
  const filteredResult = mergedList.filter((item: any) => {
    const search = searchValue.toLowerCase();
    return (
      (item.type === 'category' && item.name.toLowerCase().includes(search)) ||
      (item.type === 'product' && item.title.toLowerCase().includes(search))
    );
  });
